...
| google_csv | Создает файл my_output.json с результатами |
...
### Запуск
Указываем флаг для получения нужного формата. Если флаг не будет указан, по умолчанию, создает файл my_output.html с результатами
```sh
